 ...
 public function user_reg()
{
    ...
    $this->form_validation->set_rules('picture', '', 'callback_ps');
     ...
     if ($this->form_validation->run()  == false) {
         ...
         else
         {
            ...
            $config['file_name']=rand().$_FILES['picture']['name'];
            $this->load->library('upload', $config);   
            $this->upload->initialize($config);
            $this->upload->do_upload('picture');
            $imagedetails= $this->upload->data();
            $pictures='uploads/'.$imagedetails['file_name']; 
   ...
   $allowed_mime_type_arr = array('image/gif','image/jpeg','image/pjpeg','image/png','image/x-png');
        ...
        if(isset($_FILES['picture']['name']) && $_FILES['picture']['name']!=""){
            if(in_array($mime, $allowed_mime_type_arr)){
                ...
                $this->form_validation->set_message('ps', 'Please select only jpeg,pjpeg,png,x-png,gif');
                return false;
            }
        }else{
            $this->form_validation->set_message('ps', 'Please choose a file to upload.');
            return false;
        }
    }


 public function user_reg1()
{
    ...
    $this->form_validation->set_rules('email', 'Email');
     ...
     if ($this->form_validation->run()  == false) {
         ...
         else
         {
            ...
            //$config['max_width']    = '1024';
            //$config['max_height']   = '768';
            $config['file_name']=rand().$_FILES['JointSignature']['name'];
            $this->load->library('upload', $config);   
            $this->upload->initialize($config);
            $this->upload->do_upload('JointSignature');
            $imagedetails= $this->upload->data();
            $JointSignature='upload/JointSignature/'.$imagedetails['file_name']; 
