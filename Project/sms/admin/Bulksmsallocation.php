...
<div class="table_column" style="width:80px;">Sender_ID</div>
<div class="table_column" style="width:150px;">Mobile No</div>
<div class="table_column" style="width:150px;">Message</div>
...
<div class="clear"></div>
	 ...
	 include("pagination/function.php");
                                             $page = (int) (!isset($_GET["page"]) ? 1 : $_GET["page"]);
                                                    ...
                                                    $statement = "bulk_sms where status='Sending' order by s_id ASC"; //you have to pass your query over here
...
//$row1=$fetch_message->fetch();	*/
				...
				while($row1=$res->fetch()) :
                        ...
                        <td style='max-width:150px;padding-left:43px'>
						<a href='../".$row1['phone']."' target='download/".$row1['phone']."' download>Download</a>
						</td>
						
                        <td style='max-width:200px;padding-left:43px''><textarea class='form-control' id='hide_name1' rows='5' id='comment'>".$row1['msg']."</textarea></td>
						
						
			 <td style='max-width:100px;padding-left:55px'><p id='hide_name1'>".$row1['s_date']."</p></td>";
						if($row1['status']=='Sending'){
							echo "<td style='max-width:120px;padding-left:15px'><span style='color:red'>".$row1['status']."</td>";
						}
				echo "<td style='max-width:180px;padding-left:55px'><a href='edit-bulksms.php?edit=".$row1['s_id']."' role='button' class='btn btn-info' >Update Status</a></td>";		
					...
					<?php if(!empty($row1['phone'])) { echo '<a  href="'.$row1['phone'].'" target="_blank" style="margin-right:25px"></a>'; } ?>
...
<input name="cmddelete" id="cmddelete" type="submit" class="small_grey_btn marginright" value="Delete Selected Schedule" onclick="return confirmDelete();">&nbsp;
   ...
   //Here you are missing below statement
		...
		if($sql){
			...
			echo"<meta http-equiv=\"refresh\" content=\"0; URL= Bulksmsallocation.php\">";
		}
