...
<div class="table_column" style="width:150px;">Sender_ID</div>
...
<div class="clear"></div>
		...
		<div class="white_row">
		<table id="example" class="table_column" style="width:540px">
	 ...
	 include("pagination/function.php");
                                             $page = (int) (!isset($_GET["page"]) ? 1 : $_GET["page"]);
                                                    ...
                                                    $statement = "send_sms where status='Delivered' order by s_id desc"; //you have to pass your query over here
			...
			/*$fetch_message=$con->prepare("select * from send_sms where status='Delivered' order by s_id ASC  ");
...
//$row1=$fetch_message->fetch();		 
				...
				while($row1=$res->fetch()) :
                        ...
                        <td style='max-width:50px;padding-left:65px'><p id='hide_name1'>".$i++."</p></td>
						<td style='max-width:150px;padding-left:35px'><p id='hide_name1'>".$row1['sender_id']."</p></td>
                        <td style='max-width:180px;padding-left:35px'>
						  <textarea class='form-control' rows='5' id='hide_name1' id='comment'>".$row1['phone']."</textarea></td>
						
                        <td style='max-width:150px;padding-left:15px'><textarea class='form-control' id='hide_name1' rows='5' id='comment'>".$row1['msg']."</textarea></td>
						
						
			 <td style='max-width:70px;padding-left:65px'><p id='hide_name1'>".$row1['sms_date']."</p></td>";
						if($row1['status']=='Delivered'){
							echo "<td style='max-width:100px;padding-left:55px'><span style='color:red'>".$row1['status']."</td>";
						}
...
<input name="cmddelete" id="cmddelete" type="submit" class="small_grey_btn marginright" value="Delete Selected Schedule" onclick="return confirmDelete();">&nbsp;
   ...
   //Here you are missing below statement
		...
		if($sql){
			...
			echo"<meta http-equiv=\"refresh\" content=\"0; URL= mis.php\">";
		}
