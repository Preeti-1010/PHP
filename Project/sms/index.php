...
<div class="login_box">
    ...
    <div class="login_form_box">
      ...
      <div class="input_box">
      	<ul>
            <li><input name="sa_name" id="txtEmail" type="text" class="input_fields_login" placeholder="Username"></li>
            <li><input name="sa_password" id="txtPassword" type="password" class="input_fields_login" placeholder="Password"></li>
		...
		<div class="fright fpasword"><a href="index.php?forgot_pass">Forgot password?</a> </div>
		        <div class="fleft"><input name="u_login" id="cmdLogin" type="submit" value="login" class="input_button" onclick="return validate();"></div>
      </div>
      </form><br><br>
	  <?php
                if(isset($_GET['forgot_pass'])){
                    ...
                    <input type='text' class='form-control input_fields_login' name='f_name' placeholder='Enter registered  username' /><br>
                ...
                if(isset($_POST['f_pass'])){
                    $name=$_POST['f_name'];
                    $select_c=$con->prepare("select * from profile where sa_name='$name'");
                     $select_c->setFetchMode(PDO::FETCH_ASSOC);
                        $select_c->execute();
                        $row_c=$select_c->fetch();
                        
                        $c_name=$row_c['sa_name'];
                        $c_email=$row_c['sa_email'];
                        $c_pass=$row_c['sa_password'];      
...
<?php 
        if(isset($_POST['u_login'])){
            ...
            $select_user=$con->prepare("select * from profile where sa_name='$sa_name' AND sa_password='$sa_password'");
            $select_user->setFetchMode(PDO::FETCH_ASSOC);
            $select_user->execute();
            $data=$select_user->fetch();
            if($data['sa_name']!=$sa_name AND $data['sa_password']!=$sa_password){
                echo "<script>alert('Invalid Admin Username or Password');</script>";
            }
            elseif($data['sa_name']==$sa_name AND $data['sa_password']==$sa_password){
                
                $_SESSION["sa_name"]=$data['sa_name'];
                $_SESSION["sa_email"]=$data['sa_email'];
                
                $_SESSION["sa_password"]=$data['sa_password'];
